...
public class VanillaTextField extends AbstractComponent implements IInteractionHandler, IOnOffState {
    ...
    /**
     * If this value is true along with enabled, keyTyped will process the keys.
     */
...
//    private GuiPageButtonList.GuiResponder guiResponder;
    ...
    public VanillaTextField(int x, int y, int width, int height) {
        super(x, y);
        this.x = x;
        this.y = y;
        this.width = width;
...
//    }
    ...
    /**
     * Deletes the given number of characters from the current cursor's position, unless there is currently a selection, in which case the
     * selection is deleted instead.
     */
...
//                j1 = fontRenderer.drawStringWithShadow(s.substring(j), (float) j1, (float) i1, i);
    ...
    /**
     * Sets the position of the selection anchor (the selection anchor and the cursor position mark the edges of the selection). If the
     * anchor is set beyond the bounds of the current text, it will be put back inside.
     */
    ...
    public EnumActionResult onClicked(int mouseX, int mouseY, EMouseButton button, EEventType type) {
        ...
        if (this.isFocused && flag && button == EMouseButton.PRIMARY) {
            int i = mouseX - this.x;
