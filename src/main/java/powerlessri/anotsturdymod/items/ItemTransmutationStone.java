...
public class ItemTransmutationStone extends ItemBasicItem implements ITagBasedItem {
    ...
    private Iterable<BlockPos> getAffectedBlocks(World world, BlockPos pos, EnumFacing sideHit, int charge, boolean isSneaking) {
        if(isSneaking) {
            return BlockPos.getAllInBox(pos, pos);
        }

        return PosUtils.blocksOnPlane(pos, sideHit, charge);
    }
